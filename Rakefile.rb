...
#do_faces tasks
  task :copy_dlls_to_faces_lib  do #copies habanero and smooth DLLs to faces lib
	FileUtils.cp Dir.glob('temp/Habanero/trunk/bin/Habanero*.dll'), 'temp/HabaneroCommunity/Faces/trunk/lib'
	FileUtils.cp Dir.glob('temp/HabaneroCommunity/SmoothHabanero/trunk/bin/Habanero.Smooth*.dll'), 'temp/HabaneroCommunity/Faces/trunk/lib'
	end
  
    task :clean_faces do #deletes bin folder before build
	FileUtils.rm_rf 'temp/HabaneroCommunity/Faces/trunk/bin'
end

exec :checkout_faces do |cmd| #command to check out faces source using SVN
	cmd.path_to_command = "../../../Utilities/BuildServer/Subversion/bin/svn.exe"
	cmd.parameters %q(checkout "http://delicious:8080/svn/habanero/HabaneroCommunity/Faces/trunk/" "temp/HabaneroCommunity/Faces/trunk/" --username chilli --password chilli --non-interactive)
end
...
msbuild :msdo_faces do |msb| #builds faces with msbuild
	...
	msb.verbosity = "quiet"
  msb.solution = "temp/HabaneroCommunity/Faces/trunk/source/Habanero.Faces - 2010.sln"
  end
