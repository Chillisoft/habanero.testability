...
#do_smooth tasks
  task :copy_dlls_to_smooth_lib  do #copies habanero DLLs to smooth lib
	FileUtils.cp Dir.glob('temp/Habanero/branches/v2.5/bin/Habanero*.dll'), 'temp/HabaneroCommunity/SmoothHabanero/branches/v1.2/lib'
end

  task :clean_smooth do #deletes bin folder before build
	FileUtils.rm_rf 'temp/HabaneroCommunity/SmoothHabanero/branches/v1.2/bin'
end
...
exec :checkout_smooth do |cmd| #command to check out smooth source using SVN
	...
	cmd.parameters %q(checkout "http://192.168.123.23:8080/svn/habanero/HabaneroCommunity/SmoothHabanero/branches/v1.2" "temp/HabaneroCommunity/SmoothHabanero/branches/v1.2/" --username chilli --password chilli --non-interactive)
...
msbuild :msdo_smooth do |msb| #builds smooth with msbuild
	...
	msb.path_to_command = "C:/Windows/Microsoft.NET/Framework64/v4.0.30319/MSBuild.exe"
	msb.verbosity = "quiet"
    msb.solution = "temp/HabaneroCommunity/SmoothHabanero/branches/v1.2/source/SmoothHabanero.sln"
  end
  
  
#do_faces tasks
  task :copy_dlls_to_faces_lib  do #copies habanero and smooth DLLs to faces lib
	FileUtils.cp Dir.glob('temp/Habanero/branches/v2.5/bin/Habanero*.dll'), 'temp/HabaneroCommunity/Faces/branches/v2.5/lib'
	FileUtils.cp Dir.glob('temp/HabaneroCommunity/SmoothHabanero/branches/v1.2/bin/Habanero.Smooth*.dll'), 'temp/HabaneroCommunity/Faces/branches/v2.5/lib'
	end
  
    task :clean_faces do #deletes bin folder before build
	FileUtils.rm_rf 'temp/HabaneroCommunity/Faces/branches/v2.5/bin'
end

exec :checkout_faces do |cmd| #command to check out faces source using SVN
	cmd.path_to_command = "../../../../Utilities/BuildServer/Subversion/bin/svn.exe"
	cmd.parameters %q(checkout "http://delicious:8080/svn/habanero/HabaneroCommunity/Faces/branches/v2.5/" "temp/HabaneroCommunity/Faces/branches/v2.5/" --username chilli --password chilli --non-interactive)
end
...
msbuild :msdo_faces do |msb| #builds faces with msbuild
	...
	msb.verbosity = "quiet"
  msb.solution = "temp/HabaneroCommunity/Faces/branches/v2.5/source/Habanero.Faces.sln"
  end
...
#do_testability tasks
  task :copy_dlls_to_testability_lib  do #copies habanero and smooth DLLs to testability lib
	...
	FileUtils.cp Dir.glob('temp/HabaneroCommunity/SmoothHabanero/branches/v1.2/bin/Habanero.Smooth*.dll'), 'lib'
	end
  
    task :clean_testability do #deletes bin folder before build
	FileUtils.rm_rf 'temp/HabaneroCommunity/Habanero.Testability/branches/v1.2/bin'
end
